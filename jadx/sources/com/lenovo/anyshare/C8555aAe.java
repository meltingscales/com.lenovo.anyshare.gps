package com.lenovo.anyshare;

import android.content.res.ColorStateList;

/* renamed from: com.lenovo.anyshare.aAe  reason: case insensitive filesystem */
/* loaded from: classes7.dex */
public class C8555aAe {

    /* renamed from: a  reason: collision with root package name */
    public static final int f18299a = 16842910;
    public static final int b = 16842912;
    public static final int c = 16842919;

    public static ColorStateList a(int i) {
        int i2 = i - (-805306368);
        return new ColorStateList(new int[][]{new int[]{-16842910, 16842912}, new int[]{-16842910}, new int[]{16842912, 16842919}, new int[]{-16842912, 16842919}, new int[]{16842912}, new int[]{-16842912}}, new int[]{i - (-520093696), C21155uhc.x, i2, C21155uhc.K, i2, C21155uhc.K});
    }

    public static ColorStateList b(int i) {
        int i2 = i - (-1728053248);
        return new ColorStateList(new int[][]{new int[]{-16842910, 16842912}, new int[]{-16842910}, new int[]{16842919, -16842912}, new int[]{16842919, 16842912}, new int[]{16842912}, new int[]{-16842912}}, new int[]{i - (-1442840576), -4539718, i2, i2, i | (-16777216), -1118482});
    }
}
