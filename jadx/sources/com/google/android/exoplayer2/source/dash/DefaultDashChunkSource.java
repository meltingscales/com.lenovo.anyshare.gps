...
public class DefaultDashChunkSource implements DashChunkSource {
    ...
    public static final class RepresentationHolder {
        ...
        public RepresentationHolder(long j, int i, Representation representation, boolean z, boolean z2, TrackOutput trackOutput) {
            ...
            if (mimeTypeIsRawText(str)) {
                ...
                if (o.ad.equals(str)) {
                    ...
                    int i2 = z ? 4 : 0;
                    if (z2) {
                        emptyList = Collections.singletonList(Format.createTextSampleFormat(null, o.W, 0, null));
                    } else {
        ...
        public static boolean mimeTypeIsRawText(String str) {
            return MimeTypes.isText(str) || o.Z.equals(str);
        }

        public static boolean mimeTypeIsWebm(String str) {
            return str.startsWith("video/webm") || str.startsWith(o.s) || str.startsWith(o.R);
        }
    ...
    public static Chunk newMediaChunk(RepresentationHolder representationHolder, DataSource dataSource, int i, Format format, int i2, Object obj, long j, int i3, long j2) {
        ...
        return new ContainerMediaChunk(dataSource, new DataSpec(rangedUri.resolveUri(str), rangedUri.start, rangedUri.length, representation.getCacheKey()), format, i2, obj, segmentStartTimeUs, representationHolder.getSegmentEndTimeUs((i5 + j) - 1), j2, j, i5, -representation.presentationTimeOffsetUs, representationHolder.extractorWrapper);
    }

    private long resolveTimeToLiveEdgeUs(long j) {
        return this.manifest.dynamic && (this.liveEdgeTimeUs > b.b ? 1 : (this.liveEdgeTimeUs == b.b ? 0 : -1)) != 0 ? this.liveEdgeTimeUs - j : b.b;
    }

    private void updateLiveEdgeTimeUs(RepresentationHolder representationHolder, long j) {
        this.liveEdgeTimeUs = this.manifest.dynamic ? representationHolder.getSegmentEndTimeUs(j) : b.b;
    }
    ...
    public void getNextChunk(MediaChunk mediaChunk, long j, long j2, ChunkHolder chunkHolder) {
        ...
        if (playerTrackEmsgHandler == null || !playerTrackEmsgHandler.maybeRefreshManifestBeforeLoadingNextChunk(msToUs)) {
            ...
            if (segmentCount == 0) {
                ...
                chunkHolder.endOfStream = !dashManifest.dynamic || this.periodIndex < dashManifest.getPeriodCount() - 1;
                return;
            }
            ...
            if (segmentCount == -1) {
                ...
                if (j5 != b.b) {
                    firstSegmentNum = Math.max(firstSegmentNum, representationHolder.getSegmentNum(nowUnixTimeUs - C.msToUs(j5)));
                }
            ...
            if (mediaChunk == null) {
                nextChunkIndex = Util.constrainValue(representationHolder.getSegmentNum(j2), j7, j6);
            } else {
            ...
            if (j8 <= j6 && (!this.missingLastSegment || j8 < j6)) {
                chunkHolder.chunk = newMediaChunk(representationHolder, this.dataSource, this.trackType, this.trackSelection.getSelectedFormat(), this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), j8, (int) Math.min(this.maxSegmentsPerLoad, (j6 - j8) + 1), mediaChunk == null ? j2 : -9223372036854775807L);
                return;
            }
            DashManifest dashManifest2 = this.manifest;
            if (dashManifest2.dynamic) {
                z = true;
                if (this.periodIndex >= dashManifest2.getPeriodCount() - 1) {
                    z = false;
                }
            } else {
                z = true;
